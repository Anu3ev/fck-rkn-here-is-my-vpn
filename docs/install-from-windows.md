...
This guide assumes a new Ubuntu 24.04 x86-64 VPS and Windows 10 or 11 with PowerShell and OpenSSH. Complete the [preflight checklist](preflight.md) first.

Replace every value inside `<ANGLE_BRACKETS>` before running a command. PowerShell commands run on Windows; Bash commands run inside the VPS SSH session.
...
Leave `PANEL_ALLOWED_CIDR` empty if your administrative IP changes frequently. The script prompts twice for a new `vpnadmin` sudo password. Save it in a password manager; it is not stored in Git.
